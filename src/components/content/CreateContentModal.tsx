...
export default function CreateContentModal({
  ...
  const handleSubmit = async () => {
    ...
    if (values.text.length === 0) {
      toast.error("디지털 컨텐츠를 추가해주세요.");
      return;
    }

    toast.promise(createContent(values), {
      loading: "디지털 컨텐츠를 추가하는 중입니다.",
      success: () => {
        onClose();
  ...
  const formRules: Rule[] = [
    contentType === "BARCODE"
      ? {
          ...
          message: "바코드 형식(숫자)의 디지털 컨텐츠를 입력해주세요.",
        }
      : { required: true, message: "디지털 컨텐츠를 입력해주세요." },
  ];
      ...
      <Form form={form} layout="vertical" onFinish={handleSubmit}>
        <Alert
          ...
          message={
            <>
              <p className="mb-3">
                디지털 컨텐츠를 추가할때 엑셀 또는 메모장에서 엔터로 구분된
                문자를 붙여넣기 할 수 있습니다.
              </p>
              <Image
                ...
                <div key={field.key}>
                  ...
                  <div className="flex gap-2">
                    <Form.Item
                      ...
                      rules={formRules}
                    >
                      <Input placeholder="디지털 컨텐츠 내용을 입력해주세요." />
                    </Form.Item>
                    <Button
                      type="primary"
                      danger
